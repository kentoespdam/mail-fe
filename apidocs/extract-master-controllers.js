...
const controllerNameMap = {
	"quick-message-controller": "quick-message",
	"mail-type-controller": "mail-type",
	"mail-category-controller": "mail-category",
	"allowed-file-type-controller": "file-rule",
	"document-type-controller": "document-type",
};
...
for (const [pathUrl, pathMethods] of Object.entries(masterData.paths)) {
...
const outputDir = path.join(__dirname, "master");
if (!fs.existsSync(outputDir)) {
	fs.mkdirSync(outputDir, { recursive: true });
}

// Get all schemas from the original spec
const allSchemas = masterData.components?.schemas || {};
...
for (const [controllerTag, paths] of Object.entries(controllerPaths)) {
	...
	const controllerSpec = {
		openapi: masterData.openapi,
		info: masterData.info,
		servers: masterData.servers,
		security: masterData.security,
		paths: paths,
		components: {
			schemas: filteredSchemas,
			securitySchemes: masterData.components?.securitySchemes,
		},
	};
